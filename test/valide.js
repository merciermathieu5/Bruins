...
(async () => {
  ...
  const teuvo = S.SECOURS_ROSTER.find(j => j.nom === 'Teuvo Taravainen');
  egal(teuvo.salaire, 5750000, 'Salaire Taravainen');
  egal(teuvo.ct, 1, 'Contrat Taravainen 1 an');
  egal(S.SECOURS_Y21.length, 21, '21 patineurs dans la référence Y21');
  const petanY21 = S.SECOURS_Y21.find(x => x.nom === 'Nicolas Petan');
  egal(petanY21.pts, 74, 'Petan 74 points en Y21');
  ...
  egal(profilDe('Teuvo Taravainen').profil, 'Elite', 'Taravainen → Elite');
  egal(profilDe('Teuvo Taravainen').mat, 'ELITE', 'Taravainen → matrice ELITE');
  tableauEgal(profilDe('Teuvo Taravainen').stats, ['shotpct','gwg','ppg','pts','pmrang'],
    'Stats évaluées Elite = tableau 20 (PCTG, GWG, PP, P, +/-)');
  egal(profilDe('Nicolas Petan').profil, 'Playmaker', 'Petan → Playmaker');
  tableauEgal(profilDe('Nicolas Petan').stats, ['assists','pts'], 'Stats Playmaker = A, P');
  egal(profilDe('Evgeny Svechnikov').profil, 'Power Forward', 'Svechnikov → Power Forward');
  egal(profilDe('Brendan Brisson').profil, 'Prospect Playmaker', 'Brisson (24 ans) → Prospect Playmaker');
  egal(profilDe('Brendan Brisson').mat, 'PLAYMAKER', 'Prospect Playmaker → même matrice PLAYMAKER');
  egal(profilDe('Roman Josi').profil, 'DEliteQB', 'Josi → DEliteQB');
  egal(profilDe('Ben Thomas').profil, 'DEliteShutdown', 'Thomas → DEliteShutdown');
  egal(profilDe('Linus Ullmark').profil, 'Starter Goalie', 'Ullmark (OV 81) → Starter Goalie');
  tableauEgal(profilDe('Linus Ullmark').stats, ['hs','svpct','qggp','ming'], 'Stats Starter = HS, SV%, QG/GP, MIN');
  egal(profilDe('Alex Lyon').profil, 'Backup Goalie', 'Lyon (33 ans, OV 79) → Backup Goalie (règle 6.2.5)');
  tableauEgal(profilDe('Alex Lyon').stats, ['mp','qggp','psv'], 'Stats Backup = MP, QG/GP, Psv');
  ...
  const four = S.fourchetteRecote(teuvo, 10); // 31 ans → ModA-10 → base 60
  egal(four.base, 60, 'Base = 60 + ModA(-10) + ModS(10)');
  egal(four.min, -1, 'Pire jet (61) → -1');
  egal(four.max, 0, 'Meilleur jet (100) → 0');

  console.log('— Priorité des seuils : rangée personnalisée > Y17 ; surcharge = Mémorable seul');
  const jk = joueurDe('Teuvo Taravainen');
  ...
  const fixtureXtra = [
    ...
    '    [Teuvo Taravainen](https://x/#Teuvo Taravainen) BOSTON          C   81  36  54  90 225  26 1937 204 2.32',
    '    [Alex Tuch](https://x/#Alex Tuch) ANAHEIM         RW  77  27  32  59 183  16 1503 243 2.38',
    '    [* Backup_RW](https://x/#Backup_RW) BOSTON          LW   3   0   0   0   0   0    0   0 0.00'
  ].join('\n');
  const x = S.parseXtra(fixtureXtra, 'BOSTON');
  egal(x.length, 2, 'Deux patineurs BOSTON extraits (Anaheim exclu)');
  egal(x[0].hits, 204, 'MEÉ de Taravainen depuis XtraStats');
  egal(x[0].mp, 1937, 'Minutes de Taravainen depuis XtraStats');
  ...
  ok(doc.querySelector('#ficheEquipe').textContent.includes('BOSTON'), 'Fiche d\'équipe affichée');
  ...
  egal(masse, 80342000, 'Masse salariale des 23 pros = 80 342 000 $');
  ...
  console.log('— Divers');
  egal(S.matchsEquipe(), 0, 'Fiche BOSTON 0-0-0 (amorçage) → 0 match d\'équipe');
  ...
  if (ETAT){
    ...
    const jPetan = S.SECOURS_ROSTER.find(x2=>x2.nom==='Nicolas Petan');
    const prodRepli = S.productionDe(jPetan);
    ok(!!prodRepli && prodRepli._xtraSource===true, 'Production servie par XtraStats (drapeau _xtraSource)');
    egal(prodRepli?.pts, 74, 'Points lus depuis XtraStats en repli');
  ...
  const prodY21 = S.productionDe(S.SECOURS_ROSTER.find(x2=>x2.nom==='Nicolas Petan'));
  egal(prodY21?._modeY21, true, 'Production servie par les données Y21 intégrées');
  egal(prodY21?.pts, 74, 'Points Y21 de Petan');
  const cartePetan = [...doc.querySelectorAll('#progGrille .joueur-carte')]
    .find(c=>c.querySelector('.jc-nom').textContent==='Nicolas Petan');
  ok(!!cartePetan, 'Carte de Petan rendue en mode Y21');
  ok(cartePetan.textContent.includes('Mode vérification Y21'), 'Note du mode Y21 sur la carte');
  ok(cartePetan.querySelectorAll('.badge-etat.memorable').length >= 2,
     'Petan Mémorable en passes et en points (56 A > 46 ; 74 PTS > 73, Playmaker OV 81)');
  ok(!cartePetan.textContent.includes('proj.'), 'Aucune projection affichée (saison complète)');
  btnY21.click();
